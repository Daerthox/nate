...
describe('tagging API', () => {
    ...
    describe('tagElement()', () => {
        ...
        it('should tag the element targeted by the selector', async () => {
            ...
            const actionType = await page.evaluate(() => document.getElementById('unique-tag')?.getAttribute('nate-action-type'))
            expect(actionType).toBe('click')

            const dictionaryKey = await page.evaluate(() => document.getElementById('unique-tag')?.getAttribute('nate-dict-key'))
    ...
    describe('tagElementWithContent()', () => {
        it('should throw an error when no element with the specified content matches the selector', async () => {
            const [page, browser] = await setupPage()
            expect(() => tagElementWithContent(page, '#non-exisiting-id', {}, '')).rejects.toThrow()
            expect(() => tagElementWithContent(page, '#unique-tag', {}, 'non-existing-content')).rejects.toThrow()

            await browser.close()
        })
        it('should throw an error when more than one element with the specified content matches the selector', async () => {
            const [page, browser] = await setupPage()
            expect(() => tagElementWithContent(page, '.multi-tag', {}, 'multi content')).rejects.toThrow()
            expect(() => tagElementWithContent(page, 'div', {}, 'multi content')).rejects.toThrow()

            await browser.close()
        })
        it('should tag the element targeted by the selector if it has the specified content', async () => {
            const [page, browser] = await setupPage()
            await tagElementWithContent(page, '.multi-tag', {
                'nate-dict-key': 'test',
